...
/**
 * Modern panel-style dialog for AI-assisted data extraction.
 * Replaces the older AssistantExtractDialog with a layout consistent with
 * AssistantChatPanel and AssistantGenericPanel: collapsible context area at top,
 * extraction results table in the center, prompt input at the bottom.
 *
 * @author jens
 */
public class AssistantExtractPanel extends JDialog {

    private static final Logger log = Logger.getLogger(AssistantExtractPanel.class.getName());
    ...
    private JTable tblExtractedKeys;
    ...
    public AssistantExtractPanel(ArchiveFileBean selectedCase, AssistantConfig config, AiCapability c, AssistantInputAdapter inputAdapter, JFrame parent, boolean modal) {
        super(parent, modal);
        this.initialize(selectedCase, config, c, inputAdapter);
    }

    public AssistantExtractPanel(ArchiveFileBean selectedCase, AssistantConfig config, AiCapability c, AssistantInputAdapter inputAdapter, JDialog parent, boolean modal) {
        super(parent, modal);
        this.initialize(selectedCase, config, c, inputAdapter);
    }
    ...
    private void initialize(ArchiveFileBean selectedCase, AssistantConfig config, AiCapability c, AssistantInputAdapter inputAdapter) {
        ...
        // Hide context toggle if there is no input text
        if (!hasInputText) {
    ...
    private void buildUI() {
        ...
        lblRequestType = new JLabel("Extrahieren");
        ...
        // --- Context content: input text (collapsed by default) ---
        ...
        pnlContextContent.add(scrollInputText, BorderLayout.CENTER);
        ...
        // ========== CENTER: Extraction results table ==========
        tblExtractedKeys = new JTable();
        tblExtractedKeys.setAutoCreateRowSorter(true);
        tblExtractedKeys.setModel(new DefaultTableModel(
                new Object[][]{},
                new String[]{"übernehmen", "Attribut", "extrahierter Wert"}
        ) {
        ...
        scrollTable = new JScrollPane(tblExtractedKeys);
        scrollTable.setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8));
        ...
        addComponentListener(new java.awt.event.ComponentAdapter() {
            ...
            public void componentResized(java.awt.event.ComponentEvent evt) {
                ComponentUtils.storeDialogSize(AssistantExtractPanel.this);
            }

            @Override
            public void componentShown(java.awt.event.ComponentEvent evt) {
                FrameUtils.centerDialogOnParentMonitor(AssistantExtractPanel.this, AssistantExtractPanel.this.getOwner().getLocation());
            }
        });
    ...
    // ==================== Business Logic (ported from AssistantExtractDialog) ====================
    ...
    private void startBackgroundTask() {
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<>() {
            ...
            protected void done() {
                ...
                if (status != null) {
                    if (status.isError()) {
                        // error is silently ignored as in the original dialog
                    } else {
                ...
                protected void done() {
                    ...
                    if (status != null) {
                        ...
                        if (status.isError()) {
                            ...
                            ThreadUtils.showErrorDialog(AssistantExtractPanel.this, "Transkription fehlgeschlagen: " + status.getStatusDetails(), DesktopUtils.POPUP_TITLE_ERROR);
                        } else {
                            StringBuilder resultString = new StringBuilder();
                            for (OutputData o : status.getResponse().getOutputData()) {
                                if (o.getType().equalsIgnoreCase(OutputData.TYPE_STRING)) {
